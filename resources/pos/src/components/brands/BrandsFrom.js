...
const BrandsFrom = (props) => {
    ...
    const prepareFormData = (data) => {
        ...
        } else if(singleBrand && singleBrand.image != '') {
            formData.append('image', singleBrand?.image ?? null)
        }
        return formData;
    };
    ...
    const onSubmit = (event) => {
        ...
        formValue.image = selectImg ? selectImg : (singleBrand?.image ?? null);
        if (singleBrand && valid) {
            if (!disabled) {
                formValue.image = selectImg ? selectImg : singleBrand?.image;
