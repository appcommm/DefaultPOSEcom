...
class SaleAPIController extends AppBaseController
{
    ...
    public function bulkStore(Request $request)
    {
        ...
        foreach ($input as $sale) {
            //         $existingSales = Sale::where('id', $sale['id'])->first();
            //         if ($existingSales) {
            //             if ($sale['action'] == 'delete') {
            //                 $existingSales->delete();
            //             } elseif ($sale['action'] == 'update') {
            //                 $sale = $this->saleRepository->updateSale($sale['attributes'] ? $sale['attributes'] : $sale, $existingSales->id);
            //             }
            //         } else {
            if(!isset($sale['action'])) {
                $sale = $this->saleRepository->storeSale($sale);
            }
        }
        //     }
        // }

        // BrandResource::notUsingWithCollection();
