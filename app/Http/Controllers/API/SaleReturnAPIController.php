    ...
    public function bulkStore(Request $request)
    {
        ...
        if (count($input) == 0) {
            ...
            foreach ($input as $sale) {
                $existingSales = SaleReturn::where('id', $sale['id'])->first();
                if ($existingSales) {
                    if ($sale['action'] == 'delete') {
                        $existingSales->delete();
                    } else {
                        $sale = $this->saleReturnRepository->updateSaleReturn($sale['attributes'], $existingSales->id);
                    }
                } else {
                    $sale = $this->saleReturnRepository->storeSaleReturn($sale['attributes']);
                }
            }
        }
