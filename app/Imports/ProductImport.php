            ...
            try {
                ...
                if ($row[18] == 'Single') {
                    $mainProduct = MainProduct::create([
                        ...
                        'product_type' => $row[18] == 'Variation' ? MainProduct::VARIATION_PRODUCT : MainProduct::SINGLE_PRODUCT,
                    ]);
                } else {
                    $mainProduct = MainProduct::orderBy('id', 'desc')->first();
                    if ($mainProduct->product_type == 1) {
                        $mainProduct = MainProduct::create([
                            'name' => $row[0],
                            'code' => (string) $row[1],
                            'product_unit' => $productUnitId,
                ...
                $productData = [
                    'name' => $row[0],
                    'code' => (string) ($row[18] == 'Single' ? $row[1] : $row[21]),
                ...
                if ($row[18] == 'Variation') {
                    $variationName = Variation::whereName($row[19])->first();
                    if ($variationName) {
                        $variationNameId = $variationName->id;
                    } else {
                        $variationName = Variation::create(['name' => $row[19]]);
                        $variationNameId = $variationName->id;
                    }

                    $variationTypeName = VariationType::whereName($row[20])->first();
                    if ($variationTypeName) {
                        $variationTypeNameId = $variationTypeName->id;
                    } else {
                        $variationTypeName = VariationType::create(['variation_id' => $variationNameId, 'name' => $row[20]]);
                        $variationTypeNameId = $variationTypeName->id;
                    }
                    $variationData = [
                        'main_product_id' => $mainProduct->id,
                        'product_id' => $product->id,
                        'variation_id' =>  $variationNameId,
                        'variation_type_id' => $variationTypeNameId,
                    ];
